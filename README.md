...
```bash
rust-lightning-node-rankings$ sam build
```
...
```bash
rust-lightning-node-rankings$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
rust-lightning-node-rankings$ sam local start-api
rust-lightning-node-rankings$ curl http://localhost:3000/
```
...
```bash
rust-lightning-node-rankings$ sam logs -n HelloWorldFunction --stack-name rust-lightning-node-rankings --tail
```
